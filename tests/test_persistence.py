...
def test_reserve_ark_persists_to_db(client, test_db, mock_orchestrator):
    ...
    # Verify in database
    db_ark = test_db.query(ARKRecord).filter_by(ark=data["ark"]).first()
...
def test_update_ark_to_draft(client, test_db, mock_orchestrator):
    ...
    update_response = client.put(
        ...
        json={
            ...
            "metadata": {"title": "Test Resource", "author": "Test Author"},
        },
    )
    ...
    assert data.get("metadata_cid") is None  # Not yet published
    
    # 3. Verify in database
    db_ark = test_db.query(ARKRecord).filter_by(ark=ark).first()
    assert db_ark.state == ARKState.DRAFT
    assert db_ark.target == "https://example.org/resource"
    assert db_ark.metadata_json["title"] == "Test Resource"
...
def test_update_ark_wrong_state(client, test_db, mock_orchestrator):
    ...
    db_ark.metadata_json = {"test": "data"}
    test_db.commit()
